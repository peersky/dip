        ...
        if (dbAuthor) {
          ...
          if (email && email !== dbAuthor.email) {
            ...
            if (
              ...
              console.warn(
                `Could not update email for author ${dbAuthor.name || dbAuthor.githubHandle} to ${email} as it's already taken.`,
              );
            } else {
          ...
          try {
            ...
            if (e.code === "P2002") {
              console.warn(
                `Failed to create author (${githubHandle || email}) due to unique constraint, likely a race condition.`,
              );
              if (githubHandle) {
...
export async function resolveMovedProposals() {
  ...
  for (const sourceProposal of proposalsToResolve) {
    ...
    // Find the repo config that matches the destination folder. This is more
    // robust than splitting/popping the path, and correctly handles files
    // moved to the root directory (where dirname is '.').
    ...
    // More reliable regex: case-insensitive, requires letters in prefix if it exists.
...
async function aggregateAndStoreForMonth(dateForMonth: Date) {
  ...
  const eligibleProposals =
    proposalCounts.total - proposalCounts.withdrawn - proposalCounts.stagnant;
  const acceptanceRate =
    eligibleProposals > 0
      ? (proposalCounts.finalized / eligibleProposals) * 100
      : 0;

  // 2. Author-centric Centralization Score
  const centralizationRate =
    authorCounts.distinct > 0
      ? (1 - authorCounts.onFinalized / authorCounts.distinct) * 100
      : 0;
