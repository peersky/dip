...
async function writeFrontmatterFixMigration() {
  ...
  if (!fs.existsSync(migrationsPath)) {
    console.error(
      "\x1b[31m%s\x1b[0m",
      `ERROR: The migrations directory does not exist at ${migrationsPath}.`,
    );
    console.error(
      "Please run 'pnpm prisma migrate dev' in the database package at least once to initialize it.",
    );
    process.exit(1);
  }
  ...
  if (!targetDirName) {
    console.error(
      "\x1b[31m%s\x1b[0m",
      `ERROR: No migration directory ending in '_${migrationName}' found.`,
    );
    console.error(
      `Please create one first by running: pnpm prisma migrate dev --create-only --name ${migrationName}`,
    );
    process.exit(1);
  }
  ...
  for (const version of allVersions) {
    ...
    const { content } = matter(version.rawMarkdown);
...
`;
    ...
    console.log(
      "Analysis complete. No proposal versions with frontmatter were found. Your data is already clean!",
    );
  }
}
