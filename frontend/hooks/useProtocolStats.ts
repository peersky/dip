...
const fetchAllProtocolsStats = async (): Promise<AllProtocolsStatsResult> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.statistics?.tracksBreakdown) {
          ...
          Object.keys(result.statistics.tracksBreakdown).forEach((track) =>
            tracks.add(track),
          );
        }
      }
  ...
  console.log("Fetched protocolsData in hook:", protocolsData);

  return {
    protocolsData,
    allTracks: Array.from(tracks)
      .filter(
        (track) => track !== "Unknown Type" && track !== "SKIP_THIS_PROPOSAL",
      )
      .sort(),
