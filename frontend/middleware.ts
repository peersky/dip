...
const MAIN_DOMAIN = process.env.NEXT_PUBLIC_BASE_URL
  ...
  : "dip.box";
const PROTOCOL_SUBDOMAINS = [
  "ethereum",
  "rollup",
  "starknet",
  "arbitrum",
  "polygon",
]; // Add other protocol subdomains here
...
export function middleware(request: NextRequest) {
  ...
  if (!hostname) {
    ...
    if (!hostname) {
      console.warn(
        "Middleware: Hostname not found in headers or URL. Skipping rewrite.",
      );
  ...
  if (
    PROTOCOL_SUBDOMAINS.includes(pathSubdomain) ||
    pathSubdomain === AUTH_SUBDOMAIN
  ) {
    return NextResponse.next();
  }
  ...
  if (hostname.endsWith(".localhost")) {
    ...
    if (parts.length === 2) {
      ...
      if (PROTOCOL_SUBDOMAINS.includes(subdomain)) {
        ...
        const newPath =
          currentPath === "/" ? `/${subdomain}` : `/${subdomain}${currentPath}`;
