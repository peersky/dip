    ...
    <Container size="sm" py="xl">
      <Stack gap="md" align="center">
        ...
        <Text size="sm" c="dimmed" ta="center">
          Please switch to a supported network to continue using the
          application.
        </Text>
...
export const Web3Provider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [hadWagmiChainChanged, setHadWagmiChainChanged] =
    useState<boolean>(false);
  const [privyWalletChainId, setPrivyWalletChainId] = useState<number | null>(
    null,
  );
    ...
    (async () => {
      try {
        if (authenticated && user && wallets.length > 0) {
          const embedWallet = wallets.find(
          ...
          const activeSupportedChain = SUPPORTED_CHAINS.find(
            (chain) => chain.id === privyWalletChainId,
          );
          if (activeSupportedChain !== undefined) {
            if (wagmiAccount.status === "connected") {
              if (wagmiAccount.address !== wallet.address) {
                //if privy and wagmi accounts are not in sync
                const connections = getConnections(wagmiConfig);
                const connectionWithPrivyEmbeddedWallet = connections.find(
                  (connection) =>
                    connection.accounts.includes(wallet.address as Address),
                );
  ...
  return (
    ...
    >
      {SUPPORTED_CHAINS.find((chain) => chain.id === privyWalletChainId) ===
        undefined && privyWalletChainId !== null ? (
