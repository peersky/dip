...
export function getTenantInfo(hostname: string): TenantInfo {
  ...
  if (
    parts.length >= 3 &&
    parts[parts.length - 1] === "box" &&
    parts[parts.length - 2] === "dip"
  ) {
    const subdomain = parts[0];
    return {
      subdomain,
  ...
  console.warn(
    `Unrecognized hostname format: "${cleanHostname}". Defaulting to main domain. Consider refining getTenantInfo for this case.`,
  );
  return {
    subdomain: "main",
    protocol: "main",
...
export function getProtocolConfig(protocol: string) {
  ...
  > = {
    ...
    ethereum: {
      ...
      proposalPrefix: "EIP",
      description:
        process.env.NEXT_PUBLIC_ETHEREUM_REPO_DESCRIPTION ||
    ...
    rollup: {
      ...
      proposalPrefix: "RIP",
      description:
        process.env.NEXT_PUBLIC_ROLLUP_REPO_DESCRIPTION ||
    ...
    starknet: {
      ...
      proposalPrefix: "SNIP",
      description:
        process.env.NEXT_PUBLIC_STARKNET_REPO_DESCRIPTION ||
...
/**
 * Constructs the full URL for a given protocol's subdomain.
 * This handles different environments (development vs. production) and
 * correctly formats the URL for both the main domain and specific subdomains.
 *
 * @param subdomain The subdomain of the protocol (e.g., "ethereum", or "main").
 * @returns The full, navigable URL (e.g., "http://ethereum.localhost:3000" or "https://dip.box").
 */
export function getProtocolUrl(subdomain: string): string {
  const isDevelopment = process.env.NODE_ENV === "development";
  // For production, pull from an env var or default to dip.box. For dev, it's always localhost.
  const baseDomain =
    process.env.NEXT_PUBLIC_BASE_DOMAIN ||
    (isDevelopment ? "localhost:3000" : "dip.box");
  const protocol = isDevelopment ? "http" : "https";

  // The "main" subdomain should not be part of the URL, it should resolve to the base domain.
  if (subdomain === "main") {
...
export function getAllProtocols() {
  return [
    getProtocolConfig("ethereum"),
    getProtocolConfig("rollup"),
    getProtocolConfig("starknet"),
    getProtocolConfig("arbitrum"),
    getProtocolConfig("polygon"),
  ];
}
