...
async function inspectProposals() {
  ...
  try {
    ...
    const invalidProposals: { path: string; issues: string[] }[] = [];
    ...
    if (invalidProposals.length === 0) {
      ...
      console.log(
        `Found ${invalidProposals.length} proposals with incomplete metadata:\n`,
      );
    ...
    console.error(
      `FATAL: A critical error occurred during the inspection: ${error.message}`,
      error,
    );
    process.exit(1);
  } finally {
