"use strict";
// dip/crawler/src/merge-duplicate-authors-v3.ts
...
// --- Step 2: Disjoint Set (Union-Find) data structure for merging clusters ---
class DisjointSet {
    constructor(ids) {
        this.parent = new Map();
    ...
    union(id1, id2) {
        ...
        if (root1 !== root2) {
            // Simple union: make root1 the parent of root2
...
// --- Main script logic ---
async function main() {
    console.log("Starting v3 duplicate author cleanup job...");
    // 1. Fetch all authors
    ...
    for (const author of allAuthors) {
        ...
        if (author.githubHandle) {
            ...
            if (!byHandle.has(key))
                byHandle.set(key, []);
            byHandle.get(key).push(author);
        }
        if (author.email) {
            const key = author.email.toLowerCase();
            if (!byEmail.has(key))
                byEmail.set(key, []);
            byEmail.get(key).push(author);
        }
    }
    // 3. Use Disjoint Set to iteratively merge clusters
    const authorIds = allAuthors.map(a => a.id);
    const disjointSet = new DisjointSet(authorIds);
    const allGroups = [...byName.values(), ...byHandle.values(), ...byEmail.values()];
    let wasChanged = true;
    while (wasChanged) {
        wasChanged = false;
        for (const group of allGroups) {
            if (group.length > 1) {
                for (let i = 1; i < group.length; i++) {
                    const changed = disjointSet.union(group[0].id, group[i].id);
                    if (changed) {
                        wasChanged = true;
                    }
                }
    ...
    if (duplicateGroups.length === 0) {
        console.log("✅ No duplicate authors found after v3 analysis. Database is clean!");
        return;
    }
    ...
    for (const group of duplicateGroups) {
        ...
        try {
            ...
            }, { timeout: 120000 }); // 2 minute timeout for very large transactions
        }
        catch (error) {
...
main()
    .catch((e) => {
    console.error("A fatal error occurred during the v3 author cleanup job:", e);
    process.exit(1);
})
    .finally(async () => {
