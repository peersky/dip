...
async function main() {
    ...
    if (!firstVersion || !lastVersion) {
        console.log("No proposal versions found in the database. Cannot generate historical snapshots. Exiting.");
        return;
    }
    // Start iterating from the beginning of the month of the very first commit.
    let currentDate = new Date(firstVersion.commitDate.getUTCFullYear(), firstVersion.commitDate.getUTCMonth(), 1);
    const lastCommitDate = lastVersion.commitDate;
    console.log(`Generating monthly snapshots from ${currentDate.toISOString()} to ${lastCommitDate.toISOString()}`);
    ...
    while (currentDate <= lastCommitDate) {
        ...
        const snapshotDate = new Date(Date.UTC(currentDate.getUTCFullYear(), currentDate.getUTCMonth() + 1, // Move to the first day of the next month
        ...
        for (const repoConfig of dip_core_1.repositories) {
            if (repoConfig.enabled) {
                try {
                    ...
                    const existingSnapshot = await prisma.protocolStatsSnapshot.findUnique({
                        where: {
                            protocol_year_month: {
                    ...
                    if (existingSnapshot) {
                        console.log(`SKIPPING: Snapshot for ${repoConfig.protocol} for ${year}-${month} already exists.`);
                        continue; // Move to the next protocol
                    }
                    // If no snapshot exists, calculate and store a new one.
                    await (0, dip_core_1.calculateAndCacheStatistics)(repoConfig.protocol, snapshotDate);
                }
                catch (error) {
                    console.error(`ERROR: Failed to generate snapshot for ${repoConfig.protocol} for month ending ${snapshotDate.toISOString()}: ${error.message}`);
...
main()
    .catch((e) => {
    console.error("A fatal error occurred during the snapshot generation job:", e);
